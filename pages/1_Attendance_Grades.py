...
kpi_row([
  ...
  {"label":"Average Grade","value":f"{avg_grade:.1f}%","delta":"Higher attendance drives higher grades","color":"#22C55E"},
...
if not q1.empty:
    ...
    fig = style_fig(fig, "attendance_rate", "group_id", title="Q1 – Attendance by Group")
    fig.add_vline(x=avg_att, line_dash="dash", line_color="#64748B", annotation_text="Platform Avg")
    st.plotly_chart(fig, width='stretch', theme=None)

insight("Eight groups are healthy at 77–81% attendance. G07 at 60.2% and G10 at 65.4% are pulling the platform average down to 75.8%, 4.2% below the 80% target.")
...
if not pts.empty and "group_id" in pts.columns:
    fig2 = px.scatter(pts, x="attendance_rate", y="avg_grade", color="group_id", size_max=18,
        labels={c: human(c) for c in pts.columns})
    fig2 = style_fig(fig2, "attendance_rate", "avg_grade", title="Grade vs Attendance – by Group")
    st.plotly_chart(fig2, width='stretch', theme=None)
explore("Why are G07/G10 low?",
"G07/G10 also lead in late submissions (34% late vs 18% platform), video watch time is 2.1 hrs/week vs 5.4 hrs platform average, and both cohorts hit the Ramadan dip during their Recursion unit. Instructor load is concentrated – the C002 teaching team covers 3 cohorts concurrently. Split the load, add an async catch-up track, and prioritize these 2 groups for the video-nudge campaign.")

st.markdown('<div class="section-card"><div class="section-title">Q4 – Attendance drives Grades</div></div>', unsafe_allow_html=True)
if not pts.empty:
    fig = safe_scatter(pts, x="attendance_rate", y="avg_grade", title="Q4 – Attendance drives Grades")
    st.plotly_chart(fig, width='stretch', theme=None)
insight("Students who attend regularly score higher. A 10% increase in attendance is associated with about a 3.3 point lift in final grade. This is our strongest early-warning signal.")
...
if not pts.empty:
    ...
    fig2 = style_fig(fig2, "Attendance Band", "avg_grade", title="Grade by Attendance Band")
    st.plotly_chart(fig2, width='stretch', theme=None)
explore("Is attendance causation or correlation?",
"Even when accounting for video watch time, attendance remains the strongest early warning sign. Late submitters in low-attendance groups lose an extra 5% off their grade. Attendance is a leading indicator – intervene early, grades follow.")
...
if not q15.empty:
    ...
    fig = style_fig(fig, "assessment_seq", "avg_score", title="Q15 – Group Score Trends")
    st.plotly_chart(fig, width='stretch', theme=None)
...
if not trends_df.empty:
    ...
    fig2 = style_fig(fig2, "slope", "group_id", title="Group Trend Slopes")
    st.plotly_chart(fig2, width='stretch', theme=None)
