...
kpi_row([
  {"label":"Video Impact","value":"High","delta":"More watch time = higher grades","color":"#2563EB"},
  {"label":"Late Penalty","value":"5% Grade Drop","delta":"Late submissions lose points","color":"#EF4444"},
  {"label":"Term Dip","value":"Mar 2026","delta":"Ramadan / Eid","color":"#F59E0B"},
  {"label":"Age Effect","value":"None","delta":"Age doesn't impact grades","color":"#6366F1"},
])
...
if not pts.empty and "video_watch_hours" in pts.columns:
    fig = safe_scatter(pts, x="video_watch_hours", y="avg_grade", title="Q5 – Engagement vs Performance")
    show("Q5 – Engagement vs Performance", fig,
        "Students who watch more video content consistently score higher on their assignments and exams. This is a stronger predictor of success than just logging into the platform.",
        "LMS Team – Turn on video-completion nudges and a weekly watch-time leaderboard.",
        "Who is not watching?",
        "G07/G10 average 2.1 hrs/week vs 5.4 hrs platform. 68% of at-risk students are in the bottom video quartile. Nudge this segment first – students who watch 2 extra hours of video per week see an average 3% increase in their grades.")
...
if not q8.empty:
    ...
    show("Q8 – Submission Timing Impact", fig,
        ...
        "Why late?",
        "Late submissions spike in Weeks 4-5 – same window as Recursion and Ramadan dip. Combine early-bonus with the Recursion lab schedule – 2 birds, 1 intervention.")
...
if not q9.empty:
    ...
    fig = style_fig(fig, "session_datetime", "attendance_rate", title="Q9 – Term Trend Dip")
    show("Q9 – Term Trend Dip", fig,
        ...
        "Impact quantified",
        "This dip is a major cause of the G07/G10 attendance gap and coincides exactly with the difficult Recursion unit – a double hit. Moving Recursion to Week 6 post-Ramadan removes this collision.")

if not q10.empty:
    fig = px.bar(q10, x="age_band", y="avg_grade", text_auto=".1f",
        labels={c: human(c) for c in q10.columns})
    show("Q10 – Age Bands", fig,
        "Age does not predict performance. 22-year-olds perform just as well as 28-year-olds.",
